...
Uses ``pydantic-settings`` so every setting is read from the environment
(and from a local ``.env`` file if present) **at instantiation time**,
then cached via ``get_settings()``.
"""
...
class Settings(BaseSettings):
    ...
    # مدة صلاحية الجلسة: 7 أيام (604800 ثانية)
